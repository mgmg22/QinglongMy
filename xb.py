...
def filter_list(tr):
    ...
    if any(sub in title.lower() for sub in xykNameList) and any(sub in title.lower() for sub in cxkWhiteList):
        ...
        return False
    commonBlackList = [
        "定位", "部分", "东北", "徽", "限上海", "限深圳", "北京", "天津", "重庆", "深圳地区", "老家",
        "山东", "福建", "江苏", "云南", "江西", "河北", "广东", "吉林", "湖北", "河南", "陕西", "湖南", "四川", "宁夏",
        "广西", "辽宁", "甘肃", "内蒙古", "青海", "贵州", "山西",
        "厦门", "南京", "东莞", "广州", "南海", "苏州", "中山", "常州", "青岛", "成都", "武汉", "合肥", "揭阳", "无锡",
        "济南", "大连", "石家庄", "泉州", "丹东", "茂名", "长沙", "泰州", "郑州", "惠州",
        # ----卡----
        "万事达", "visa", "车主卡", "刷卡达标", "北分",
        "陆金所",
        "缤纷生活",
        "浦大喜奔",
        ...
        "ump.cmpay.com/info", "和包生日",
        "深i工",
        # ----问题----
        "么", "问题", "问问", "问下", "谢谢", "请问", "问一下", "请教", "求", "咋", "怎样", "咨询", "赐教", "啥",
        "有问", "行不", "何解", "不行", "原因", "帮忙看", "哪来的", "都多少", "是多少", "是不是", "有谁", "大佬",
        "果熟", "有果", "油果", "彦祖", "亦菲", "多不", "谁有", "有没", "如何", "预算", "你们都", "几号", "到底",
        "多少出", "哥哥们",
        # ----数码----
        "华为", "huawei", "荣耀手机", "mipay", "果子", "苹果", "iphone", "airpods", "pm", "亚瑟", "大疆", "数据",
        "k70",
        # ----生活居家----
        "內衣", "拖鞋", "洞洞鞋", "购物袋", "布袋", "婴", "小孩", "孩子", "辣妈", "无线", "牙膏", "拉拉裤", "伞",
        "锅", "电动车", "盔", "清风", "纸", "维达",
        "杯", "椅", "菌", "石头", "奥妙", "家政", "冈本", "南极人", "巾", "染", "一次性", "蚊香", "容声", "沐浴",
        "力士",
        # ----风险----
    ...
    highBlackList = [
        "【顶】",
        # ----玩法----
        "需要邀请", "助力", "人团", "拼团", "调研", "申请x", "互助", "攒能量", "组队", "组团",
        "首单", "盲盒", "月黑风高", "互换", "入会", "买1送1", "买一送一",
        # ----网购----
        "支", "件", "/袋", "/盒", "/斤", "箱", "罐", "xl", "货", "瓶",
        "降", "9.9",
        ...
        "呢", "呀!", "吧？", "啦 ！", "了。。", "了吧", "了啊", "啊~", "。。。",
        # ----情绪----
        "卧槽", "逼", "牛b", "还好", "根本", "有点东西", "感觉", "居然", "感谢", "心态", "无语", "毛线", "怨", "真是",
        "无耻", "便宜啊", "麻烦", "不如不", "狗了", "差了", "终于", "太次了", "不想搞", "只有", "不服", "见过", "竟然",
        "太难", "看到", "发财", "咱", "口感", "妥妥", "死活", "就这", "狗屁", "感受", "没想到", "兄弟们", "后悔",
        # ----负面----
        "不能", "删了", "续费", "限制使用", "冻结", "被盗", "差评", "监控", "套牢", "猫饼", "怀疑", "不知道", "黑心",
        "没有", "想法", "网友", "挤", "上科技", "不友好", "骗", "返买", "反买", "关闭", "闲谈", "投诉", "虚假", "进群",
        "被封", "禁", "修复", "盾", "砍", "赔", "秒退", "限额", "吧里", "赖", "水贴", "吐了", "真的假的", "妹", "聊",
        "最惨",
        # ----时效----
        "以后", "即将", "过期", "长期出", "逾期", "防身", "前10", "前年", "收到短信", "上个月",
        "改规则", "几秒", "忘记", "想起", "明天", "变了", "删除", "最近", "上次",
        "可是",
        # ----end----
        "黄了", "没了", "下线", "凉了", "领不了", "领完", "我才", "没抢到", "没到", "未到账", "不到账", "凉凉",
        "抢不到", "上限", "退款", "不给", "砍单", "抢光", "不可用", "审核", "捡漏", "领不到",
        "翻车", "失败", "崩溃", "崩了", "黑号", "号黑", "拦截", "卸载", "火爆", "销户", "垃圾", "风控", "不玩了",
        # ----虚拟----
        "高德", "工行码", "吧码", "迅雷", "电子书", "车险", "模板",
        # ----不符合预期的词语----
        "的水", "漏水", "纯水", "碱水", "水果", "水雾", "吸水", "精萃水", "精华水", "净水", "补水", "花露水",
    ...
    lowBlackList = [
        ...
        # ----食品----
        "三只松鼠", "海底捞", "火锅", "烧烤", "麻辣烫", "馋嘴", "卤", "炖", "火腿", "爪",
        "蛋白", "豆浆", "维生素", "麦片", "飞鹤", "粥", "面", "小麦", "米线", "粉", "粮", "裙带菜",
        # ----生鲜----
        "巧乐兹", "梦龙", "可生食", "羊肉", "虾", "粽", "笋", "大闸蟹", "海参",
        "榴莲", "梨", "柠檬", "香菇", "鲜花", "莓", "玫瑰", "酸菜", "雪糕", "冰淇淋", "柿",
        # ----饮料----
        "饮料", "果汁", "百岁山", "农夫山泉", "矿泉水", "茅台", "酒", "窖", "特仑苏", "椰子", "茶叶", "观音",
        "奶茶", "coco", "奈雪", "蜜雪", "茶百道", "古茗", "库迪",
        # ----美妆个护----
        "珀莱雅", "雅诗兰黛", "毛戈平", "屈臣氏", "大宝", "金纺", "立白", "科颜氏", "林清轩", "欧莱雅", "苏菲", "蔻",
        "洗", "眉", "唇", "泥", "日抛", "护理", "采销", "卫生", "敏感肌", "美妆蛋", "保湿", "美白", "精粹", "海蓝",
        "口罩", "OLAY",
        # ----其他实物----
        "佛", "机油", "宠物", "翡翠", "轮胎", "图书",
        # ----品牌----
    ...
    whiteList = [
        ...
        # "同程", "携程", "途牛",
        "话费", "移动", "和包", "电信", "q币", "扣币",
        "麦当劳", "肯德基", "必胜客", "星巴克", "瑞幸", "朴朴", "喜茶", "霸王茶姬", "百果园",
        "礼品卡", "星礼卡",
        "深圳通", "网上国网",
    ]
