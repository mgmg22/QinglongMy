...
def filter_list(tr):
    ...
    commonBlackList = [
        ...
        "济南", "大连", "石家庄",
        # ----卡----
        "平安X", "平安x", "平安银行X", "平安信", "平安心", "平安银行信",
        "中行x", "中行信", "中行心", "中国银行信",
        "民生x", "民生信",
        "邮储x", "邮储X", "邮储信", "邮储联名", "邮储美团联名", "闪光卡",
        "农行x",
        "交行x", "光大x", "交通银行数币", "交行数币",
        "农行信", "交行信",
        "光大信", "光大心用卡", "光大麦当劳", "阳光惠生活",
        "广发", "恒丰", "汇丰", "农商",
        "浦大喜奔", "缤纷生活", "万事达",
        "宝石山", "众邦", "联名卡", "农行刷卡金", "北分",
        "刷卡达标",
    ...
    highBlackList = [
        ...
        "有没有", "有问", "行不", "何解", "不行", "原因", "帮忙看", "哪来的", "都多少", "是多少", "是不是",
        "果熟", "有果", "油果", "彦祖", "多不",
        # ----符号----
        "吗？", "吧？", "啥", "呢", "链接？", "么？", "呀!", "啊？", "啦 ！", "用？", "？？", "不？", "次？", "准？",
        "了？", "了?", "了。。", "了啊", "啊~",
        # ----情绪----
        "卧槽", "牛逼", "牛B", "还好", "根本", "有点东西", "感觉", "居然要", "感谢", "心态", "无语", "奇怪", "毛线",
        "无耻", "便宜啊", "麻烦", "不如不", "狗了", "差了", "终于", "太次了", "不想搞",
        # ----负面----
        "不能", "删了", "续费", "限制使用", "被盗", "差评", "监控", "套牢", "猫饼", "怀疑", "不知道", "真的假的",
        "我没有", "想法", "网友", "挤", "上科技",
        # ----时效----
        "以后", "即将", "过期", "长期出", "逾期", "发货", "退货", "防身", "前10", "前年", "收到短信",
        # ----end----
        "黄了", "没了", "下线", "凉了", "领不了", "领完了", "我才", "没抢到", "没到", "未到账", "凉凉", "抢不到",
        "上限", "退款", "不给退",
        "翻车", "失败", "崩溃", "黑号", "号黑", "拦截", "卸载", "火爆", "销户", "垃圾", "风控", "不玩了",
        # ----虚拟----
        "风险", "美元", "提额", "保险", "开通", "境外", "秒批", "下卡", "送您一张会员礼品卡", "工行码", "吧码",
        "迅雷", "唯品会小程序", "爱奇艺红包", "电子书", "贷", "征信",
        # ----不符合预期的词语----
        "漏水", "纯水", "碱水", "水果", "水雾", "酒水", "吸水", "精萃水", "净水", "补水", "花露水", "热水",
        "玻璃水", "cm", "ml", "ML", "口水", "缩水", "水龙", "水润", "水牛", "水枪", "香水", "水壶",
        "沥水", "水乳", "卸妆水", "防水", "饮水", "水泡", "水感",
        "签到红包",
        "朋友圈",
    ]
    lowBlackList = [
        ...
        # ----生活居家----
        "內衣", "拖鞋", "洞洞鞋", "购物袋", "婴儿", "小孩", "孩子", "辣妈", "无线", "牙膏",
        "杯", "椅", "清风", "纸", "菌", "石头",
        # ----食品生鲜----
        "巧乐兹", "三只松鼠",
        "奶茶", "果汁", "奈雪", "蜜雪", "农夫山泉", "矿泉水", "茅台", "五粮液", "习酒", "窖", "特仑苏", "可生食",
        "火锅", "羊肉", "虾", "烧烤", "麻辣烫", "馋嘴", "卤", "炖", "粽",
...
'''
        ...
        with open("log/log_xb.md", 'w', encoding='utf-8') as f:
            f.write(content)
    else:
        print("暂无线报！！")
