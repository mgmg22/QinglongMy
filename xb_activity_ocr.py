...
for logger_name in ['paddleocr', 'ppocr']:
...
def has_white_bank_name(content):
    return any(sub in content.lower() for sub in cxkWhiteList)


def has_black_xyk_name(content):
    xykNameList = ["xing/用卡", "信用卡", "xing用卡", "信用k", "心用卡", "性用卡", "xyk", "信y卡", "深圳"]
    return any(sub in content.lower() for sub in xykNameList) and has_white_bank_name(content)
...
def check_ocr(img_url):
    result = ocr.ocr(get_image_path(img_url), cls=True)
    ocr_txt = ""
    if result[0]:
        for line in result[0]:
            txt, confidence = line[1]
            ocr_txt += txt
        print("----ocr识别结果：" + '\t\t' + ocr_txt)
        return has_black_xyk_name(ocr_txt)


def get_image_path(img_url):
    # 从 URL 中提取文件名
    file_name = os.path.basename(img_url)
    cached_path = os.path.join(temp_folder, file_name)
...
def filter_list(tr):
    ...
    if not any(sub in title.lower() for sub in whiteList) and not has_white_bank_name(title):
        return False
    ...
    for img in img_tags:
        ...
        if has_white_bank_name(title) and check_ocr(img_url):
            print("----ocr识别不合法，已忽略" + '\t\t' + href + '\t\t' + img_url)
            return False
    print(title + '\t\t' + href)
    item = {
        'title': title,
